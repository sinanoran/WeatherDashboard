...
public class WeatherEndpointTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    [InlineData("")]
    [InlineData("   ")]
    public async Task SetDefaultLocation_Returns400_WhenCityIsMissing(string city)
    {
    ...
    [Fact]
    public async Task SetDefaultLocation_Returns400_WhenCityExceedsMaxLength()
    {
        string city = new string('a', 101);
        HttpResponseMessage response = await _client.PutAsJsonAsync("/api/location/default", new { city });
