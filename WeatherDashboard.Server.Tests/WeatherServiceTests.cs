...
public class WeatherServiceTests
{
    ...
    [Fact]
    public async Task GetWeatherAsync_DelegatesToProvider_WithCorrectCity()
    {
        _provider
            .Setup(provider => provider.GetWeatherAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((WeatherInfo?)null);

        await _sut.GetWeatherAsync("Paris");

        _provider.Verify(provider => provider.GetWeatherAsync("Paris", It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    [Fact]
    public async Task GetWeatherAsync_CorrectlyConvertsTemperature()
    {
        WeatherInfo info = new WeatherInfo("Test", 100, 50, 5, "hot", "01d");
        _provider
            .Setup(provider => provider.GetWeatherAsync("Test", It.IsAny<CancellationToken>()))
            .ReturnsAsync(info);

        WeatherResponse? result = await _sut.GetWeatherAsync("Test");

        Assert.NotNull(result);
        Assert.Equal(100, result.TemperatureC);
        Assert.Equal(212, result.TemperatureF);
    }
}
